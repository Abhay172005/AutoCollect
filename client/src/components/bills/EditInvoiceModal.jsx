...
const EditInvoiceModal = ({ bill, onClose, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    cumulativeAmount: bill.cumulativeAmount || 0,
    notes: bill.notes || ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await billService.updateBill(bill._id, {
        ...formData,
        billAmount: Number(formData.billAmount),
        balanceAmount: Number(formData.balanceAmount),
        cumulativeAmount: Number(formData.cumulativeAmount),
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-dark-800 rounded-2xl max-w-md w-full shadow-2xl animate-scale-in flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto">
          <form id="edit-invoice-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Balance Amount *</label>
                <input required type="number" min="0" step="0.01" className="input-field" value={formData.balanceAmount} onChange={e => setFormData({ ...formData, balanceAmount: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cumulative Amount</label>
                <input type="number" min="0" step="0.01" className="input-field" value={formData.cumulativeAmount} onChange={e => setFormData({ ...formData, cumulativeAmount: e.target.value })} />
              </div>
            </div>
            <div>
        ...
        <div className="p-6 border-t border-gray-200 dark:border-dark-700 flex gap-3 justify-end shrink-0 bg-gray-50 dark:bg-dark-800/50 rounded-b-2xl">
          ...
          <button type="submit" form="edit-invoice-form" disabled={loading} className="btn-primary">
